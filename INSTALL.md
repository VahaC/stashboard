...
# Image version: `latest`, or pin one, e.g. 5.9.0.
...
| `STASHBOARD_TAG` | only to pin | `latest` tracks the newest build. Pin (e.g. `5.9.0`) for a reproducible deploy. |
...
To move to a specific version, set `STASHBOARD_TAG=5.9.0` in `.env` first, then
