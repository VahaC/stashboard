...
/// <summary>
/// V5.7 — in-memory implementation of <see cref="IContainerExecSessionRegistry"/>.
...
/// the V5.3 host-shell registry.
/// </summary>
public sealed class ContainerExecSessionRegistry(IOptions<ContainerExecOptions> options) : IContainerExecSessionRegistry
{
    ...
    public IDisposable? TryAcquire(Guid userId, Guid connectionId, out string? rejection)
    {
        ...
        lock (_gate)
        {
            ...
            {
                rejection = $"You already have {userCount} exec session(s) open (limit {maxUser}).";
                return null;
            }
            ...
            {
                rejection = $"This host already has {hostCount} exec session(s) open (limit {maxHost}).";
                return null;
            }
    ...
    private sealed class Lease(ContainerExecSessionRegistry owner, Guid userId, Guid connectionId) : IDisposable
    {
