...
- **Installable PWA + web push + custom card order** (V10.6) — install Stashboard to your home screen (standalone app-shell that loads offline), receive **web-push** notifications on your device when a service goes down or an update lands (auto-provisioned VAPID keys, per-device subscriptions, same per-source toggle/throttle as the other channels), and **drag-and-drop** your service cards into a Custom order that persists across devices. Requires HTTPS (or `localhost`) for install + push
...
✅ **V10.0 — Notification channels beyond email/Telegram (Apprise)** _(shipped in 10.0.0)_ — every notification (service offline, Docker update available, Proxmox updates pending, node alert) can now fan out to **Discord, ntfy, Gotify, Slack, generic webhooks and 90+ other services** via **[Apprise](https://github.com/caronc/apprise-api)** — one HTTP POST per event to **your own** Apprise API (or the public stateless `/notify` endpoint), so Stashboard never has to hand-roll a provider API. App-wide config lives on the **Notifications** settings page (mirroring the editable-SMTP model): a master switch **off by default**, an Apprise base URL, and one **Apprise URL** per line (`discord://`, `ntfy://`, `gotify://`, …) stored DB-backed with the URLs **encrypted at rest** and never returned (presence flag + non-secret schemes only); changes apply without a restart, and a **Send test** button fires a sample through each target reporting **per-target** success/failure. It's a fully **independent channel**: per-watch / per-host **Apprise** toggles (surfaced disabled until configured, exactly like the Telegram toggle) and a per-channel throttle key **stamped only after a successful send**, so a transient Apprise outage retries next tick and never drops the existing email/Telegram delivery — and vice-versa. The config (URLs encrypted) + per-target toggles round-trip through backup/restore. **Out of scope:** running a built-in Apprise instance, and per-notification routing rules (every notification fans out to all configured channels). See the [CHANGELOG](./CHANGELOG.md).

✅ **V10.1 — Uptime history & analytics** _(shipped in 10.1.0)_ — the health check stops forgetting. Stashboard now keeps a bounded, append-only **uptime history** per service, surfaced on a dedicated **Uptime** tab in the service modal: **uptime % over 24 h / 7 d / 30 d**, a **response-time sparkline** (hand-rolled inline SVG — no chart library), and an **incident log** of past Down→Up spans with durations (the still-down span flagged *ongoing*). History is tracked **per URL** — the main and the additional URL each get their own metrics. Rows are written by the background scan **and** by **Check now**, but **only on a status change or on a sampled cadence** (default 15 min), so a steady service never bloats the table; a background prune drops anything past the **retention window** (default 90 days). Both the retention window and the sample cadence are new knobs on **Settings → Health checks** (DB-backed, seeded from `STASHBOARD_HealthCheck__*`). Owner-scoped read endpoints expose the rolled-up metrics + incidents and a paginated raw-events feed (a foreign service 404s). The history is runtime-derived telemetry and is **not** included in backup/restore (it re-builds on the live instance), consistent with the rest of the runtime status. **Out of scope:** SLA reports / CSV exports, uptime-% alert thresholds (offline alerting stays in the V5.6 failure-threshold logic), and long-term downsampling. See the [CHANGELOG](./CHANGELOG.md).
...
✅ **V10.6 — PWA, web push & custom card ordering** _(shipped in 10.6.0)_ — Stashboard becomes a daily home-screen surface. A **web app manifest** + a **hand-rolled service worker** make it **installable** with a standalone display and brand app-icons; the SW caches the app shell so the login/dashboard chrome **loads offline** (data still fetches when online — no offline data sync), serves fingerprinted `/assets` cache-first, leaves `/api` / `/uploads` / WebSockets alone, and updates silently via a versioned cache. **Web push** is a full V10.0-style channel: the browser subscribes **per device**, and the **same** service-offline, Docker-update, Proxmox-update and node-alert notifications fan out to push — mirroring the Apprise per-source toggle + independent throttle key (stamped only after a successful, non-transient send). **VAPID keys auto-provision once** into the persisted-secrets dir beside the SQLite DB (matched pair, reused on restart), exactly like the encryption/JWT secrets; delivery + payload encryption use the `WebPush` library. Subscriptions are endpoint-unique per device — the channel is "on" for a user precisely when they have **≥1 subscription** (no separate flag) — and an endpoint the push service reports **gone (404/410)** is **pruned** on the next send while transient failures retry. Manage devices + **Send test** from **Settings → Notifications**. Finally, a **Custom** dashboard sort mode adds pointer-based (mouse + touch) **drag-and-drop** with **two independent card orders** (global when ungrouped, within-category when grouped) plus **reorderable category groups**, so toggling *Group by category* never scrambles the other mode; the order **round-trips through backup/restore**. Push subscriptions are device-bound bearer material and are **not** exported; **web push + install require a secure context (HTTPS or `localhost`)** and the UI degrades gracefully otherwise. **Out of scope:** native app-store apps, offline data caching / write queue, and reordering the Docker/Proxmox pages. See the [CHANGELOG](./CHANGELOG.md).
