...
/**
 * V6.6 — Settings → LXC console. The app-wide master switch for the browser
 * Proxmox LXC console (an interactive shell inside an LXC, reached by SSHing to
 * the Proxmox host and running `pct exec`). Like the host terminal and
 * container exec, the page leads with the risks and spells out every condition
...
export function ProxmoxConsoleSettings() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ kind: 'ok', text: enabled ? 'LXC console enabled server-wide.' : 'LXC console disabled.' })
    } catch (error: unknown) {
      const { globalError } = parseApiErrors(error)
      setMessage({ kind: 'err', text: globalError ?? 'Failed to save LXC-console setting.' })
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="account-page account-stack">
      <h1 className="text-2xl font-semibold">LXC console (Proxmox)</h1>

      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <SquareChevronRight className="h-5 w-5" /> Proxmox LXC console
          </CardTitle>
          <CardDescription>
        ...
        <CardContent>
          ...
          <form onSubmit={submit} className="account-form account-form-spaced">
            <label className="account-checkbox-label">
              ...
              />
              Enable the LXC console server-wide
            </label>
          ...
          <section className="host-shell-settings-section">
            ...
            <ol className="host-shell-settings-conditions">
              ...
              <li>
                ...
                <strong> “Allow LXC console”</strong>. It's off by default and set per host, so enabling one host
                never enables another.
              </li>
              <li>
                <strong>SSH is configured</strong> on the host (host / username / private key) — the console SSHes
                in and runs <code>pct exec</code>.
              </li>
            </ol>
            <p>
              If any condition is missing, the ticket request is refused server-side and no WebSocket is opened —
              the gate isn't just a hidden button. The LXC must also be running.
            </p>
          </section>
