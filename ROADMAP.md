...
### ✅ Shipped (10.2.0) Phase V10.2 — Public status page

**Shipped in 10.2.0.** A shareable, read-only status page — the headline feature of the
monitoring tools Stashboard's audience already knows (Uptime Kuma). A new `StatusPageEntity`
+ `StatusPageItemEntity` join lets a user assemble one or more pages, each a named selection of
their own services with a title, optional description and a globally-unique public **slug**. A
**publish** toggle (off by default) gates visibility: the unauthenticated `GET /api/status/{slug}`
endpoint (`PublicStatusController`, `[AllowAnonymous]`) only resolves **published** pages — an
unpublished or unknown slug 404s identically, so pages can't be enumerated. The payload is built by
`PublicStatusPageBuilder`, reusing the V10.1 `HealthCheckMetricsCalculator` (new `DailyUptime`
helper) for a 30-day uptime bar + 24 h / 7 d / 30 d uptime %, and exposes **only** whitelisted
display fields — the per-item display-name override (or service name), live status, uptime and the
history bar — **never** URLs, credentials, notes, categories, tags, ids or Docker/Proxmox internals.
The endpoint is IP-rate-limited (a new `public-status` policy reusing the existing rate-limiter) and
sends a 30 s cache header. A new **Settings → Status pages** sub-page manages create / edit /
publish / delete and copies the public link; a top-level SPA route `/status/:slug` renders the
public view (outside auth / the app shell) with an aggregate "All systems operational" banner.
Status pages + their selections + publish state are added to `BackupService` export/import (slug
regenerated on collision) and its round-trip test. **Out of scope (as planned):** custom domains /
white-labelling; subscriber digests; owner-authored incident / maintenance banners.
